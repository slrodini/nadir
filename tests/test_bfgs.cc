...
std::pair<double, double> rosenbrock(double x0, double y0)
{
   ...
   nadir::LBFGS::MetaParameters mp = nadir::LBFGS::MetaParameters{
       .max_it          = 1000,
       .eps             = 1.0e-8,
       .grad_toll       = 1.0e-14,
       .diff_value_toll = 1.0e-14,
   };
   nadir::LBFGS bfgs(mp, fn_cost, p);

   std::cout << nadir::Minimizer::print_status(bfgs.minimize()) << std::endl;

   Eigen::VectorXd diff             = (p_true - bfgs.GetParameters());
   std::pair<double, double> result = {fn(bfgs.GetParameters()), sqrt(diff.squaredNorm())};
   // bfgs.FlusToStdout();
   return result;
}
